...
func (v *NameCharacter) String() string {
	...
	return fmt.Sprintf("NameCharacter{%d bytes}", len(v.data.Bytes()))
}
...
func (v *NameCharacter) Serialize() ([]byte, error) {
	if v == nil {
		return nil, fmt.Errorf("cannot serialize because NameCharacter is nil")
	}
...
func ParseNameCharacter(src io.Reader, tag *Uint16, extended *Uint32) (*NameCharacter, error) {
	...
	if dataLength != length {
		return nil, fmt.Errorf("broken NameCharacter")
	}

	result := &NameCharacter{
		Tag:      tag,
		Extended: extended,
		data:     data,
	}
